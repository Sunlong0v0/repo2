...
// 提供公共静态文件 更健壮的：
app.use(express.static(path.join(__dirname, 'public')));  // 这里指定了 public 目录
...
async function clearDirectory(directoryPath) {
    ...
    try {
        ...
        if (!fs.existsSync(absolutePath)) {
            console.log(`${absolutePath} 目录不存在`);
            return;
        }
        ...
        if (files.length === 0) {
            console.log(`${absolutePath} 目录为空，跳过删除操作。`);
            return;
        }
        ...
        console.log(`${absolutePath} 目录已清理完成`);
    } catch (err) {
        console.error('清理目录时发生错误:', err);
    }
}
...
// 上传文件接口
app.post('/uploadPDB2', upload3.single('pdbFile2'), (req, res) => {

    if (!req.file) {
        return res.status(400).json({ success: false, message: '文件上传失败' });
    }
    const { chainName, fixedSites, numEntries, temperature, modelSelection } = req.body;
    console.log('上传参数:', { chainName, fixedSites, numEntries, temperature, modelSelection });
...
// 运行脚本接口
app.post('/run-script5', (req, res) => {
    const { chainName, fixedSites, numEntries, temperature, modelSelection } = req.body;

    // 构建命令
    const command = `bash /media/liying/11/data/ProteinMPNN/000/server.sh -j job1 -c "${chainName}" -f "${fixedSites}" -n ${numEntries} -t ${temperature} -m ${modelSelection}`;
    ...
    exec(command, (error, stdout, stderr) => {
        if (error) {
            ...
            return res.status(500).json({ success: false, message: '执行作业时出错', error: error.message });
        }
        if (stderr) {
            console.error(`命令错误输出: ${stderr}`);
            return res.status(500).json({ success: false, message: '作业执行错误', error: stderr });
        }

        console.log(`命令输出: ${stdout}`);
        res.json({ success: true, message: '脚本成功执行', output: stdout });
    });
});

// 下载结果接口
app.get('/downloadResult', (req, res) => {
    const proteinName = req.query.proteinName; // 从查询参数中获取蛋白质名称
    if (!proteinName) {
        return res.status(400).send('蛋白质名称是必需的');
    }

    // 结果文件路径
    const resultPath = `/media/liying/11/data/server/v8/results/job1/seqs/${proteinName}.fa`;

    // 检查文件是否存在
    fs.stat(resultPath, (err, stats) => {
        if (err || !stats.isFile()) {
            console.error('未找到文件:', resultPath);
            return res.status(404).send('结果文件未找到');
        }

        // 设置下载文件的名称
        const downloadName = `${proteinName}.fa`;

        // 开始文件下载
        res.download(resultPath, downloadName, (err) => {
            if (err) {
                console.error('下载错误:', err);
                return res.status(500).send('下载结果时出错');
            }
        });

        // 文件下载完成后，清理 uploads 和 results 目录
        clearDirectory('./uploads');
        clearDirectory('./results');
    });
});
